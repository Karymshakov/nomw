...
SYSTEM_PROMPT = '''You are Aida, a warm and professional guest relations assistant for Nomad Camp — a boutique hotel on the scenic south shore of Lake Issyk-Kul, Kyrgyzstan.
...
3. **Respond in the guest\'s language** — If the guest writes in Russian, reply in Russian. If they write in English, reply in English. Default to Russian if unclear.
...
COMPANY_PROFILE = '''**Nomad Camp** — A Boutique Hotel on Lake Issyk-Kul
...
def update_aiconfig_prompts(apps, schema_editor):
    AIConfig = apps.get_model('leads', 'AIConfig')
    config = AIConfig.objects.first()
    if config:
        config.system_prompt = SYSTEM_PROMPT
        config.company_profile = COMPANY_PROFILE
        config.save()
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('leads', '0029_ringcentralconfig_alter_leadactivity_activity_type'),
    ]
    ...
    operations = [
        ...
        migrations.RunPython(update_aiconfig_prompts, migrations.RunPython.noop),
    ]
