...
function CustomersPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: () => {
      toast.error(t('customers.deleteCustomerError'))
    },
  })
  ...
  return (
    <div className="flex flex-1 flex-col">
          ...
          <div className="px-4 lg:px-6">
            <div className="rounded-md border overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{t('leads.contactPerson')}</TableHead>
                    <TableHead>{t('common.email')}</TableHead>
                    <TableHead>{t('common.status')}</TableHead>
                    <TableHead>{t('customers.channels')}</TableHead>
                    <TableHead>{t('customers.contractStart')}</TableHead>
                    <TableHead>{t('customers.contractEnd')}</TableHead>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('customers.deleteCustomerTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {customerToDelete?.contact_person} — {t('customers.deleteCustomerDesc')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
